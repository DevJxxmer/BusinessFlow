...
drop policy if exists "members can manage inventory" on public.inventory_movements;
drop policy if exists "members can read finance" on public.financial_transactions;
drop policy if exists "members can manage finance" on public.financial_transactions;
drop policy if exists "members can read agenda" on public.agenda_events;
drop policy if exists "members can manage agenda" on public.agenda_events;
